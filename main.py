...
DATA_PATHS = {
    "losloop": {"feat": 'data/T_GCN_speed_input_normalized.csv', "adj": "data/T_GCN_distance_input_normalized.csv"}
}


def get_model(args, dm):
    return models.TGCN(adj=dm.adj, hidden_dim=args.hidden_dim)


def get_task(args, model, dm):
    return getattr(tasks, args.settings.capitalize() + "ForecastTask")(
...
def main_supervised(args):
    ...
    trainer = pl.Trainer.from_argparse_args(args, callbacks=callbacks,max_epochs = 20)
    trainer.fit(task, dm)
